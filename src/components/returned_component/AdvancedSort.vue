...
div
  ...
  v-dialog(v-model='advanced_sort_dialog' max-width='500px' scrollable persistent)
    v-card
      ...
      v-card-text
        v-autocomplete(v-model='product_name' :items='returned_data' label='Наименование товара' persistent-hint item-text='product_name')
        v-autocomplete(v-model='category_name' :items='returned_data' label='Kатегория' persistent-hint item-text='category_name')
        v-autocomplete(v-model='storage_name' :items='returned_data' label='Склад' persistent-hint item-text='storage_name')
        v-autocomplete(v-model='client_name' :items='returned_data' label='Клиент' persistent-hint item-text='client_name')
        v-layout
